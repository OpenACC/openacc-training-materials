  ...
  contains
    subroutine blur5(imgData, outImg, w, h, c)
      ...
      do yblock = 0, numBlocks - 1
        ...
        if ( lower < upper ) then
          !$acc update device(imgData(:,:,lower:upper)) async(mod(yblock,3))
          lastrow = upper
        endif
        !$acc parallel loop collapse(2) private(red, blue, green, ix, iy) async(mod(yblock,3))
        do y=ystart, yend
          do x=1, w
            red   = 0.0
            blue  = 0.0
          ...
          enddo
        enddo
        !$acc update self(outImg(:,:,ystart:yend)) async(mod(yblock,3))
      enddo
      !$acc wait
      !$acc end data
    end subroutine
